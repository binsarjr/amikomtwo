...
<Page class="pb-20">
	...
	<Block>
		<select bind:value={semesterSelected} on:change={refresh} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
			{#each $initKhs?.Semester || [] as semester}
				<option value={semester.Kode}>{semester.Nama}</option>
			{/each}
		</select>
		<select bind:value={tahunAkademikSelected} on:change={refresh} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
