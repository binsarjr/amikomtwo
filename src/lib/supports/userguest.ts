...
export const encryptGuestData = (mahasiswa: IBio, password: string) => {
    ...
    let exportdata: UserGuest = {
        nim: mahasiswa.Mhs.Npm, signature, nama: mahasiswa.Mhs.Nama, fitur: {
            // no used yet
            presensi: true
        }
    }
...
export const decryptGuestData = (encrypted: string) => {
    ...
    const { password } = JSON.parse(MikomSupports.Encryption.decrypt(encObject.signature, privateKey))
