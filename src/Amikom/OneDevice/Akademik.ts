...
const HasilStudi = async (bearerToken: string, xApiKey: string) => {
    const resp: ITranskripNilai = await requestAmikomOne
        .post(
            `https://ds.amikom.ac.id/api/amikomone/academic/krs/hasil_studi`,
            {
                headers: {
...
const HasilSemester = async (bearerToken: string, xApiKey: string, semester: number, tahunAkademik: string) => {
    const resp: IHasilSemester = await requestAmikomOne
        .post(
            `https://ds.amikom.ac.id/api/amikomone/academic/krs/hasil_semester`,
            {
                headers: {
                    Authorization: `Bearer ${bearerToken}`,
                    'X-Api-Key': xApiKey
