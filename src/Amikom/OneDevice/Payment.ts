...
export default {
	TicketCost: async (bearerToken: string, xApiKey: string): Promise<ITicketCost[]> => {
		const response: ResponsePage<ITicketCost[]> = await requestAmikomOne
			.get(
				`https://ds.amikom.ac.id/api/amikomone/payment/${Tokenizer(bearerToken).npm}/biaya_tiket`,
				{
					headers: {
	...
	TicketHistory: (bearerToken: string, xApiKey: string) =>
		requestAmikomOne
			.get(
				`https://ds.amikom.ac.id/api/amikomone/payment/${Tokenizer(bearerToken).npm}/histori_tiket`,
				{
					headers: {
						Authorization: `Bearer ${bearerToken}`,
						'X-Api-Key': xApiKey
	...
	History: async (bearerToken: string, xApiKey: string): Promise<IHistory[]> => {
		const response: ResponsePage<IHistory[]> = await requestAmikomOne
			.get(`https://ds.amikom.ac.id/api/amikomone/payment/${Tokenizer(bearerToken).npm}/histori`, {
				headers: {
					Authorization: `Bearer ${bearerToken}`,
					'X-Api-Key': xApiKey
				}
	...
	Bill: (bearerToken: string, xApiKey: string) =>
		requestAmikomOne
			.get(`https://ds.amikom.ac.id/api/amikomone/payment/${Tokenizer(bearerToken).npm}/tagihan`, {
				headers: {
					Authorization: `Bearer ${bearerToken}`,
					'X-Api-Key': xApiKey
				}
